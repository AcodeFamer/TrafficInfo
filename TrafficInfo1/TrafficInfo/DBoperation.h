...
void DBoperation::dataWriteToDatabase(string table_name,  vector<T>& data)
{
	...
	string SQL_insert_head = "insert into " + table_name + "(";
	for (size_t j = 0; j < name.size() - 1; j++)
	{
		SQL_insert_head = SQL_insert_head + name[j] + ",";
	}
	SQL_insert_head = SQL_insert_head + name[name.size() - 1] + ") values ";
	...
	for (size_t i = 0; i < data.size(); i++)
	{
		...
		if (count_db_write == 30)
		{
			...
			//完整的SQL插入语句
			string SQL_insert = SQL_insert_head + SQL;
