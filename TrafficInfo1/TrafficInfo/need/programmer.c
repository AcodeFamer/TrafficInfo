...
/*
 *
 * This file is not compiled, it is here only to fool the
 * "Intellisense" functionality in MSVC that these functions exist, so
 * that auto-completion works in the user's plugin file.
 * 
 * Example - in plugin.c (or whatever the name of the user file), type
 * "qpg_LNK" followed by CTRL-SPACE to see a list of possible
 * functions.
 *
 */
...
/*--- to update this file, go to programmer.h and look for "cut from here" --*/
/*--- follow the instructions, remembering to replace ";" WITH "{}" ---------*/
/*--- PASTE BELOW HERE ------------------------------------------------------*/
...
QPG float qpg_CFM_gradientAccelerationTruck,(VEHICLE *V, LINK *link){}
QPG float qpg_CFM_linkSpeed(LINK* link, VEHICLE* vehicle, Bool Accel){}
...
QPG int   qpg_DRW_layerColour(int object){}
...
char**	  qpg_NET_studyAreaNames(void){}
...
QPG Bool  qpg_RTM_arrival(LINK* link, VEHICLE* vehicle){}
QPG int   qpg_RTM_decision(LINK* link, VEHICLE* vehicle){}
QPG Bool  qpg_RTM_enable(void) {}
QPG void  qpg_RTM_nextLink(LINK* link, VEHICLE* vehicle, int nextout, LINK* *nextlink, int *newdestp){}
...
QPG void  qpg_POS_cameraPosition(int index, float *vpx, float *vpy, float *vpz, float *chx, float *chy, float *chz){}
QPG void  qpx_GUI_keyPress(int key, int ctrl, int shift, int left, int middle, int right){}
...
// re-added in 6.6
QPG BEACON* qpg_EDT_selectedBeacon(){}
QPG BUSROUTE* qpg_EDT_selectedBusRoute(){}
QPG BUSSTOP* qpg_EDT_selectedBusStop(){}
QPG CARPARK* qpg_EDT_selectedCarpark(){}
QPG DETECTOR* qpg_EDT_selectedDetector(){}
QPG LINK* qpg_EDT_selectedInwardLink(){}
QPG LINK* qpg_EDT_selectedLink(){}
QPG NODE* qpg_EDT_selectedNode(){}
QPG NODE* qpg_EDT_selectedNodeSecondary(){}
QPG int   qpg_EDT_selectedOutwardIndex(){}
QPG LINK* qpg_EDT_selectedOutwardLink(){}
QPG ZONE* qpg_EDT_selectedZone(){}

QPG BEACON* qpg_GUI_selectedBeacon(){}
QPG CARPARK* qpg_GUI_selectedCarpark(){}
QPG DETECTOR* qpg_GUI_selectedDetector(){}
QPG LINK* qpg_GUI_selectedLink(){}
QPG NODE* qpg_GUI_selectedNode(){}
QPG VEHICLE* qpg_GUI_selectedVehicle(){}
QPG ZONE* qpg_GUI_selectedZone(){}
...
// not in v6 QPX void  qpx_GUI_editorSelection(void){}
// not in v6 QPX void  qpx_GUI_testMenu(int type){}
// not in v6 QPX void  qpx_GUI_viewSelection(void){}
...
// not in v6 QPG Bool  qpg_UTL_fileClose(FILE *fp){}
// not in v6 QPG FILE* qpg_UTL_fileOpen(char *md, char *path){}
