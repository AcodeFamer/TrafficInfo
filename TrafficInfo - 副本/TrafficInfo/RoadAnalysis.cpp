...
void RoadAnalysis::analyseNode()
{
	...
	for (size_t i = 0; i<allNodeId.size(); i++)
	{
		...
		if (allNodeId[i].size() == 5)
		{
			...
			if (allNodeId[i][3] == '0' && allNodeId[i][4] == '0')
			{
				...
				int is_singal;
				//该路口信号化，写控制方案表
				if (qpg_NDE_signalised(allNodePointer[i]) == TRUE)  
				{
					is_singal = 1;
					...
					*/
					...
					//此时存在于allController的数目便是对应的控制方案编号
					int plan_index = allControlPlan.size();
	...
	//相关数据写入数据库
	db_oper.dataWriteToDatabase("nodeinfo", allLink);
